...
-(void) setShouldAppearFromTop: (BOOL) m
{
    shouldAppearFromTop = m;
}

-(void) setShouldAutoDismiss: (BOOL) m
{
    shouldAutoDismiss = m;
}

-(void) setShouldClickToDismiss: (BOOL) m
{
    shouldClickToDismiss = m;
}
...
- (void)showNextMessage
{
    if ([self.messageBarQueue count] > 0)
    {
        ...
        CGRect initRect = CGRectMake(0, screenHeight+[messageView height], [messageView width], [messageView height]);
        
        CGRect endRect = CGRectMake(0, screenHeight-[messageView height], [messageView width], [messageView height]);
        
        if(shouldAppearFromTop)
        {
            endRect = CGRectMake(messageView.frame.origin.x, messageView.frame.origin.y , [messageView width], [messageView height]+8);
        }
        
        if(!shouldAppearFromTop)
        {
            messageView.frame = initRect;
        }else
        {
        ...
        if (messageView)
        {
            ...
            /*
            [UIView animateWithDuration:kTWMessageBarManagerDismissAnimationDuration animations:^{
                [messageView setFrame:CGRectMake(messageView.frame.origin.x, messageView.frame.origin.y + [messageView height]+400, [messageView width], [messageView height])]; // slide down
            }];
            */
            
            [UIView animateWithDuration:kTWMessageBarManagerDismissAnimationDuration animations:^{
                [messageView setFrame: endRect]; // slide down
            }];
